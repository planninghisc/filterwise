...
type CornerBadgeType = '보도자료' | '부동산' | '글로벌' | 'OTC'

/** 우상단 라벨 배지 */
function CornerBadge({ type }: { type: CornerBadgeType }) {
  const style: Record<CornerBadgeType, string> = {
    보도자료: 'border-red-200 bg-red-50 text-red-700',
    부동산: 'border-green-200 bg-green-50 text-green-700',
    글로벌: 'border-sky-200 bg-sky-50 text-sky-800',
    OTC: 'border-violet-200 bg-violet-50 text-violet-800',
  }

  return (
    <span
      className={`absolute right-3 top-3 inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-medium shadow-sm ${style[type]}`}
    >
      {type}
    </span>
  )
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        {/* ── S&P DJI: S&P Global Infrastructure Index ── */}
        <Link
          href="/data/spgi-infra"
          className="relative text-left rounded-2xl border bg-white p-5 shadow-sm transition hover:shadow-md active:scale-[0.99]"
        >
          <CornerBadge type="글로벌" />
